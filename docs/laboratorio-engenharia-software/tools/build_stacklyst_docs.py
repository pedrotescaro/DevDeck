...
SCOPE_MODULES = [
    ...
    ("IA educacional", "Tutor e análise de repositório sob demanda, com provedores opcionais e limites.", "Implementado parcialmente", "src/app/api/ai; src/lib/ai"),
...
def build_vision() -> None:
    ...
    tech_rows = [
        ...
        ["IA", "Camada opcional com Ollama, Groq, Gemini ou API compatível; prompts especializados, validação e rate limit. Uso auxiliar.", "src/lib/ai; src/app/api/ai"],
        ...
        ["APIs", "REST/JSON internas; Supabase; execução de código por Wandbox/Judge0; GitHub para análise autorizada; provedores de IA opcionais.", "src/app/api; src/lib/code-runner.ts; src/lib/github.ts"],
...
ACTIVITIES = [
    ...
    {
        "id": "AN06", "name": "Utilizar assistência da IA",
        "objective": "Oferecer apoio educacional contextual sem transferir à IA decisões humanas ou competitivas.",
        "participants": "Usuário; Stacklyst; Serviço de IA externo/secundário.",
        "preconditions": "Funcionalidade disponível, pedido permitido e limites de uso não excedidos.",
        "flow": ["O usuário informa uma dúvida ou dificuldade.", "O Stacklyst prepara contexto limitado e valida o pedido.", "O sistema aplica limites e solicita apoio ao provedor configurado.", "O serviço processa o contexto e tenta gerar orientação.", "Se não houver resposta, o Stacklyst informa a indisponibilidade e oferece conteúdo existente.", "Se houver, o sistema apresenta a resposta como sugestão.", "O usuário revisa a orientação e decide os próximos passos."],
        "decisions": "Pedido permitido? Limite disponível? Provedor disponível? Resposta válida? É necessária avaliação humana?",
        "exceptions": "IA indisponível, timeout, resposta vazia/inadequada ou limite excedido. Fluxos essenciais continuam sem IA.",
        "result": "Orientação auxiliar apresentada ou fallback acionado; nenhuma decisão humana obrigatória é substituída.",
        "evidence": "src/app/api/ai; src/lib/ai; src/lib/ratelimit.ts",
        "diagram": "AN06-utilizar-assistencia-da-ia",
    },
    {
...
RF_GROUPS = [
    ...
    ("Grupo 4 — Duelos", [
        ...
        make_rf("RF020", "Avaliar resultado", "Alta", "Avaliar soluções por testes automáticos, voto e/ou avaliador humano.", "Stacklyst; Avaliador; Serviço de IA auxiliar", "RN012, RN013, RN016", "Implementado parcialmente", "src/app/api/evaluations; DuelEvaluation; DuelVote", "Soluções, testes, pontuações, feedback e análise auxiliar.", "Selecionar modalidade de avaliação e registrar decisão autorizada.", "Pontuações, feedback, vencedor e justificativas.", "IA não pode ser autoridade final quando avaliação humana for exigida."),
    ...
    ("Grupo 5 — Inteligência Artificial", [
        make_rf("RF022", "Solicitar ajuda à IA", "Média", "Enviar dúvida contextual a um provedor de IA opcional.", "Usuário; Serviço de IA", "RN013, RN024", "Implementado", "src/app/async; src/app/api/ai/chat; src/app/api/ai/trails/chat", "Mensagens e contexto permitido.", "Validar tamanho, aplicar rate limit, montar prompt e chamar provedor.", "Orientação auxiliar ou erro público controlado.", "Não enviar segredos; contexto e limites devem ser respeitados."),
        make_rf("RF023", "Gerar exercício personalizado", "Baixa", "Gerar exercício relacionado a uma dúvida ou dificuldade identificada.", "Usuário; Serviço de IA", "RN013, RN024", "Implementado parcialmente", "generateQuizAI e rota administrativa de quiz; não há fluxo completo por dificuldade individual", "Tópico, nível, dúvida e preferências.", "Solicitar saída estruturada, validar esquema e submeter a revisão/regra de publicação.", "Rascunho de exercício com resposta e explicação.", "Não publicar automaticamente sem critérios definidos."),
        make_rf("RF024", "Recomendar conteúdo", "Baixa", "Sugerir lições e atividades adequadas ao contexto do usuário.", "Usuário; Serviço de IA", "RN013, RN024", "Planejado", "não foi localizado motor persistente de recomendação", "Progresso, preferências e dúvida autorizada.", "Selecionar conteúdo existente e justificar recomendação.", "Lista de sugestões, sem bloquear o percurso.", "[ANOTAÇÃO — DECISÃO TÉCNICA PENDENTE]: dados permitidos e estratégia."),
        make_rf("RF025", "Identificar dificuldades recorrentes", "Baixa", "Detectar padrões de erro para apoiar o estudo.", "Usuário; Stacklyst", "RN013, RN024, RN027", "Planejado", "tentativas existem, mas não há agregação completa de dificuldades", "Histórico de tentativas autorizado.", "Agrupar erros sem produzir diagnóstico absoluto.", "Tópicos de atenção e sugestões de revisão.", "Definir período, consentimento, explicabilidade e retenção."),
    ]),
    ("Grupo 6 — Avaliação Humana", [
