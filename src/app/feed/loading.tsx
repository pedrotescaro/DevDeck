...
export default function FeedLoading() {
  return (
    <div
      className="mx-auto flex min-h-screen w-full max-w-[1225px] bg-dd-bg text-dd-text"
      aria-busy="true"
    >
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 border-r border-dd-border p-5 md:block xl:w-[275px]">
        <div className="flex items-center gap-3 px-3">
          <div className="dd-skeleton h-8 w-8 rounded-lg" />
          <div className="dd-skeleton h-5 w-28 rounded-full" />
        </div>
        <div className="mt-8 space-y-4 px-3">
          {Array.from({ length: 7 }).map((_, index) => (
            <div key={index} className="flex items-center gap-4 py-1">
              <div className="dd-skeleton h-6 w-6 rounded-full" />
              <div className="dd-skeleton h-3.5 w-24 rounded-full" />
            </div>
          ))}
        </div>
      </aside>
