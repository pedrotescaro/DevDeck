        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-6 bg-dd-surface border border-dd-border rounded-xl p-6 backdrop-blur-sm shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-dd-muted text-xs mt-0.5">Desafie a comunidade em problemas de algoritmos, envie soluções no editor e vote!</p>
            </div>
          </div>
        ...
        {showDuelForm && (
          <div className="rounded-xl border border-dd-border bg-dd-surface p-6 space-y-4 backdrop-blur-sm shadow-sm">
            <div className="flex items-center gap-2 border-b border-slate-900 pb-3">
              ...
              <h3 className="text-xs font-bold text-dd-muted uppercase tracking-wider">Configurar Novo Desafio</h3>
            </div>
            
            <form onSubmit={handleCreateDuel} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-bold text-dd-muted uppercase tracking-wider mb-2">Título do Duelo</label>
                  <input
                    type="text"
                    value={duelTitle}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-dd-muted uppercase tracking-wider mb-2">Linguagem Requerida</label>
                  <select
                    value={duelLanguage}
              ...
              <div>
                <label className="block text-[10px] font-bold text-dd-muted uppercase tracking-wider mb-2">Descrição e Casos de Teste</label>
                <textarea
                  value={duelBody}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {duels.length === 0 ? (
            <div className="col-span-2 rounded-xl border border-dd-border bg-dd-surface/10 p-16 text-center text-dd-muted text-xs">
              Nenhum duelo de código ocorrendo no momento. Seja o primeiro a criar um desafio acima!
            </div>
          ) : (
            duels.map((duel) => (
