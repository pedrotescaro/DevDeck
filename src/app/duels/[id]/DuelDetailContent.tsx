...
export function DuelDetailContent({ user, initialDuel }: DuelDetailContentProps) {
  ...
  const challengerPercent = totalVotes > 0 ? Math.round(((challengerSolution?.vote_count ?? 0) / totalVotes) * 100) : 50;
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-dd-bg text-dd-text antialiased">
      ...
      <div className="flex-grow flex flex-col min-w-0">
        ...
        <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-6 backdrop-blur-sm shadow-sm">
          ...
          <div>
            <h1 className="text-xl font-extrabold tracking-tight text-dd-text">{duel.problem_title}</h1>
          ...
          <div className="border-t border-slate-900 pt-4 space-y-2">
            ...
            <p className="text-xs text-dd-text leading-relaxed whitespace-pre-wrap">{duel.problem_body}</p>
          </div>
        </div>
        ...
        {duel.status === "ACTIVE" && isParticipant && (
          <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-4 backdrop-blur-sm shadow-sm">
            ...
            <p className="text-dd-muted text-xs leading-relaxed">Desenvolva sua resposta utilizando a linguagem {duel.language}. Lembre-se de cobrir todos os casos de teste citados na descrição para vencer o duelo.</p>
        ...
        {duel.status === "ACTIVE" && (
          <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-6 backdrop-blur-sm shadow-sm">
            <div className="flex items-center gap-2 border-b border-slate-900 pb-3">
              ...
              <h2 className="text-sm font-extrabold text-dd-text">Arena de Votos da Comunidade</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4 border-b md:border-b-0 md:border-r border-slate-900 pb-6 md:pb-0 md:pr-8">
                <div className="flex justify-between items-center">
                  <span className="font-bold text-xs text-dd-text">Desafiante: @{duel.challenger.username}</span>
                  <span className="text-[10px] text-dd-muted font-semibold">{challengerSolution ? "Código enviado ✅" : "Codificando... ⏳"}</span>
                </div>
                ...
                {challengerSolution ? (
                  ...
                  <div className="bg-dd-bg/40 border border-slate-850 rounded-lg p-12 text-center text-xs text-dd-muted italic">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] text-dd-muted font-semibold">
                    {duel.opponent ? (opponentSolution ? "Código enviado ✅" : "Codificando... ⏳") : "Matchmaking..."}
                  </span>
                </div>
                ...
                {opponentSolution ? (
                  ...
                  <div className="bg-dd-bg/40 border border-slate-850 rounded-lg p-12 text-center text-xs text-dd-muted italic">
                    {duel.opponent ? "Aguardando envio do código..." : "Buscando oponente na arena..."}
                  </div>
                )}
              </div>
            </div>
            ...
            {totalVotes > 0 && (
              <div className="border-t border-slate-900 pt-6 space-y-3">
                ...
                <div className="flex items-center justify-between text-[10px] font-mono font-bold text-dd-muted">
                  <span>@{duel.challenger.username} ({challengerPercent}%)</span>
                  <span>{duel.opponent ? `@${duel.opponent.username}` : "Oponente"} ({opponentPercent}%)</span>
                </div>

                <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden flex">
                  <div className="h-full bg-orange-500 transition-all duration-350" style={{ width: `${challengerPercent}%` }} />
                  <div className="h-full bg-emerald-500 transition-all duration-350" style={{ width: `${opponentPercent}%` }} />
                </div>
              </div>
            )}
          </div>
