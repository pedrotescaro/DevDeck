                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="text-xs leading-relaxed text-dd-text">
                          <span className="font-extrabold">
                            {firstUpvoter.username}
                          </span>
                          {otherCount > 0 && (
                            <> e mais <span className="font-extrabold">{otherCount}</span></>
                          )}
                          {" "}curtiram seu post <span className="text-dd-muted font-normal">· {formatTimeAgo(item.created_at)}</span>
                        </div>

                        {/* Snippet */}
                        {item.postBody && (
                          <Link href={item.link || "#"} className="block text-xs text-dd-muted leading-relaxed hover:text-dd-text transition-colors line-clamp-2">
                            {item.postBody}
                          </Link>
                        )}
                      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs font-bold text-dd-text">
                          {item.title}
                        </p>
          ...
          {showPushBanner && (
            <div className="mx-4 mt-6 p-4 rounded-xl border border-dd-border bg-dd-surface/80 backdrop-blur-md relative space-y-2.5">
              ...
              <p className="text-xs text-dd-muted leading-relaxed max-w-md">
                Ative as notificações por push para não perder nunca o que está acontecendo no DevDeck.
              </p>
              <button
                onClick={() => {
