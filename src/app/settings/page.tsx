  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const res = await fetch(`/api/profile/${authUser.user_metadata?.username || authUser.email?.split("@")[0]}`);
        if (res.ok) {
      ...
      <div className="flex-grow flex flex-col min-w-0">
        ...
        <div className="bg-dd-surface border border-dd-border rounded-xl p-5 backdrop-blur-sm shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-dd-muted text-xs mt-0.5">Atualize os dados públicos do seu perfil ou encerre sua sessão.</p>
          </div>
        </div>
        ...
        <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-6 backdrop-blur-sm shadow-sm">
          <div className="flex items-center gap-2 border-b border-dd-border pb-3">
            ...
            <h2 className="text-xs font-bold text-dd-muted uppercase tracking-wider">Informações Públicas</h2>
          </div>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-5">
            <div>
              <label className="block text-[11px] font-bold text-dd-muted uppercase tracking-wider mb-2" htmlFor="username">
            ...
            <div>
              <label className="block text-[11px] font-bold text-dd-muted uppercase tracking-wider mb-2 flex items-center gap-1.5" htmlFor="institution">
            ...
            <div>
              <label className="block text-[11px] font-bold text-dd-muted uppercase tracking-wider mb-2 flex items-center gap-1.5" htmlFor="githubUsername">
            ...
            <div>
              <label className="block text-[11px] font-bold text-dd-muted uppercase tracking-wider mb-2 flex items-center gap-1.5" htmlFor="bio">
        ...
        <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-4 backdrop-blur-sm shadow-sm">
          <div className="flex items-center gap-2 border-b border-dd-border pb-3">
            ...
            <h2 className="text-xs font-bold text-dd-muted uppercase tracking-wider">Aparência da Plataforma</h2>
          </div>
          <p className="text-dd-muted text-xs leading-relaxed">Escolha a sua preferência de exibição visual para navegar na rede.</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <div className="flex items-center justify-between w-full">
                <Moon className={`w-5 h-5 ${theme === 'dark' ? 'text-orange-400' : 'text-dd-muted'}`} />
                {theme === 'dark' && <span className="w-2 h-2 rounded-full bg-orange-500" />}
              </div>
              <div className="w-full">
                <p className="text-xs font-bold">Modo Escuro</p>
                <p className="text-[10px] text-dd-muted mt-0.5 font-semibold">Foco no código e menor fadiga ocular.</p>
              </div>
            </button>
            ...
            >
              <div className="flex items-center justify-between w-full">
                <Sun className={`w-5 h-5 ${theme === 'light' ? 'text-amber-500' : 'text-dd-muted'}`} />
                {theme === 'light' && <span className="w-2 h-2 rounded-full bg-orange-500" />}
              </div>
              <div className="w-full">
                <p className="text-xs font-bold">Modo Claro</p>
                <p className="text-[10px] text-dd-muted mt-0.5 font-semibold">Estilo limpo e alto contraste de leitura.</p>
              </div>
            </button>
          </div>
        </div>
        ...
        <div className="bg-dd-surface border border-dd-border rounded-xl p-6 space-y-4 backdrop-blur-sm shadow-sm">
          <div className="flex items-center gap-2 border-b border-dd-border pb-3">
            ...
            <h2 className="text-xs font-bold text-red-400 uppercase tracking-wider">Ações da Conta</h2>
          </div>
          
          <p className="text-dd-muted text-xs leading-relaxed">Encerre sua sessão de desenvolvimento atual no dispositivo.</p>
          <button
